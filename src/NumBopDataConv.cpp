...
//		filename_omv = filename_in + "_" + tmpStrng + ".vtk";
		filename_omv = "SelectedAllCarmen_" + tmpStrng + "_" + curr_name + ".vtk";
		...
		// here, translate coordinate to bins
		for(unsigned int i = 0; i < xy_list.size(); i++){
			for(unsigned int j = 0; j < xy_list[i].size(); j++){
				cout << "!!(" << xy_list[i][j] << "):: ";
//				string::size_type idx = src.find_last_of(" ");
//				istringstream issBase_num(src.substr(0, idx));
//				istringstream issExp(src.substr(idx + 1, 3));
//				issBase_num >> base;
//				issExp >> exp;
				istringstream xy_stream(xy_list[i][j]);
				float sx, sy;
				xy_stream >> sx;
				xy_stream >> sy;
//				cout << fixed << setprecision( 2 ) << "[" << first << ", " << last << "]";
				string lcs = boost::lexical_cast<string>(i*2);
				if(((0 <= sx)&&(sx < 119))&&((-400 < sy)&&(sy <= 0))){
					xy_list[i][j]= "0 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((119 <= sx)&&(sx < 253))&&((-124 < sy)&&(sy <= 0))){
					xy_list[i][j]= "50 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((253 <= sx)&&(sx < 409))&&((-124 < sy)&&(sy <= 0))){
					xy_list[i][j]= "100 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((409 <= sx)&&(sx <= 550))&&((-124 < sy)&&(sy <= 0))){
					xy_list[i][j]= "150 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((119 <= sx)&&(sx < 253))&&((-265 < sy)&&(sy <= -124))){
					xy_list[i][j]= "200 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((253 <= sx)&&(sx < 409))&&((-265 < sy)&&(sy <= -124))){
					xy_list[i][j]= "250 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((409 <= sx)&&(sx <= 550))&&((-265 < sy)&&(sy <= -124))){
					xy_list[i][j]= "300 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((119 <= sx)&&(sx < 253))&&((-400 <= sy)&&(sy <= -265))){
					xy_list[i][j]= "350 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((253 <= sx)&&(sx < 409))&&((-400 <= sy)&&(sy <= -265))){
					xy_list[i][j]= "400 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
				else if(((409 <= sx)&&(sx <= 550))&&((-400 <= sy)&&(sy <= -265))){
					xy_list[i][j]= "450 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
//				if(((sx < 0)||(550 > sx))&&((sy < -400)||(0 < sy))){
				else{
					xy_list[i][j]= "500 " + lcs;
					cout << "[" << xy_list[i][j] << "]";
				}
